...
app.post('/signup',async(req,res)=>{
    ...
    if(check){
        return res.status(400).json({success:false,errors:"existing user found with same email adderss"})
    }
    let cart = {};
    for(let i=0;i<300;i++){
        cart[i] = 0;
...
app.post('/login',async(req,res)=>{
    ...
    if(user){
        ...
        res.json({success:false,errors:"User Email Not Found"});
    }
}) 
...
// cerating endpoint for popular in women section
app.get('/popularinwomen',async (req,res)=>{
 ...
 app.post('/addtocart', fetchUser, async (req, res) => {
    ...
    let userData = await User.findOne({ _id: req.user.id }); // ✅ corrected
    userData.cartData[req.body.itemId] += 1;
    await User.findOneAndUpdate({ _id: req.user.id }, { cartData: userData.cartData }); // ✅ corrected
    res.send("Added");
});
